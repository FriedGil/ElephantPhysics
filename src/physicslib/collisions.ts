...
export function collision(world: World){
    for (let i = 0; i < world.dynamicBodies.length; i++){
        for (let j = 0; j < world.dynamicBodies.length; j++){
            if (i != j){
                if (touching(world.dynamicBodies[i], world.dynamicBodies[j])){
                    ...
                    const v1nNew = (v1n * (b1.mass - b2.mass) + 2 * b2.mass * v2n) / (b1.mass + b2.mass);
                    const v2nNew = (v2n * (b2.mass - b1.mass) + 2 * b1.mass * v1n) / (b1.mass + b2.mass);
        ...
        if (world.dynamicBodies[i].pos.x - world.dynamicBodies[i].shape.radius < leftbound){
            world.dynamicBodies[i].pos.x = leftbound + world.dynamicBodies[i].shape.radius;
            world.dynamicBodies[i].vel.x *= -1;
        }
        if (world.dynamicBodies[i].pos.x + world.dynamicBodies[i].shape.radius > rightbound){
            world.dynamicBodies[i].pos.x = rightbound - world.dynamicBodies[i].shape.radius;
            world.dynamicBodies[i].vel.x *= -1;
        }
        if (world.dynamicBodies[i].pos.y - world.dynamicBodies[i].shape.radius < upbound){
            world.dynamicBodies[i].pos.y = upbound + world.dynamicBodies[i].shape.radius;
            world.dynamicBodies[i].vel.y *= -1;
        }
        if (world.dynamicBodies[i].pos.y + world.dynamicBodies[i].shape.radius > downbound){
            world.dynamicBodies[i].pos.y = downbound - world.dynamicBodies[i].shape.radius;
            world.dynamicBodies[i].vel.y *= -1;
        }
    }
}
...
function touching(b1: DynamicBody, b2: DynamicBody): boolean{
    if ((b1.shape instanceof Circle || b1.shape instanceof Elephant) && ( b2.shape instanceof Circle || b2.shape instanceof Elephant)){
        const d = b1.pos.sub(b2.pos).mag();
        const r1 = b1.shape.radius;
        const r2 = b2.shape.radius;
        return d < r1 + r2;
